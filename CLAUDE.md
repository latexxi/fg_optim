...
Not a test suite — it's a sequence of numbered, narrated demos ("Run 1" through "Run 6") where each run's configuration and print-statement commentary explains what the previous run left on the table and why the next one's hyperparameters were chosen. Runs 1-5 are fixed-K weight/position optimization (Run 5's Kf=6/Kg=5 multistart is the best brute-force feasible frontier); Run 6 is the Task B topology-move demo (`grow_topology` growing Run 3's 3+2 by accepted insertions). When extending this file, follow that pattern (add a new "Run N" with an explanatory header) rather than replacing prior runs, since they double as a record of what's already been tried.
